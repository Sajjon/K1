...
func doTestGroup<HF: HashFunction, TV: WycheproofTestVector>(
	...
	#expect(key.derRepresentation.hex == group.keyDer)
	#expect(keyFromDER == key)

	let keyFromPEM = try K1.ECDSA.PublicKey(pemRepresentation: group.keyPem)
	#expect(key.pemRepresentation == group.keyPem)
	#expect(keyFromPEM == key)
	...
	outerloop: for testVector in group.tests {
		...
		do {
			...
			#expect(expectedFailure, "Test ID: \(testVector.tcId) is valid, but failed \(errorMessage).")
			continue
		}
		...
		case "valid":
			...
			#expect(isValid, "Test vector is valid, but is rejected \(testVector.tcId)")
		case "acceptable":
			#expect(isValid, "'acceptable' test vector, expected isValid to be `true`, but was not, tcID: \(testVector.tcId)")
		case "invalid":
			if isValid {
				print("❌ Test ID: \(testVector.tcId) is valid (we expected 'invalid'), but failed.")
			}
			#expect(!isValid, "Test ID: \(testVector.tcId) is valid (we expected 'invalid'), but failed.")
		default:
			Issue.record("Unhandled test vector")
		}
	}
