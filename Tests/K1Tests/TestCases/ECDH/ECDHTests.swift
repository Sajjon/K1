...
struct ECDHTests {
	...
	func ecdhX963() throws {
		...
		ab.withUnsafeBytes {
			#expect(Data($0).count == 32)
		}
		#expect(ab == ba, "Alice and Bob should be able to agree on the same secret")
	}

	@Test
	func ecdhLibsecp256k1() throws {
		let alice = K1.KeyAgreement.PrivateKey()
		let bob = K1.KeyAgreement.PrivateKey()
		#expect(alice != bob)
	...
	@Test
	func crypto_stackexchange_vector() throws {
		...
		libsecp256k1.withUnsafeBytes {
			#expect(Data($0).hex == "5935d0476af9df2998efb60383adf2ff23bc928322cfbb738fca88e49d557d7e")
		}

		#expect(ans1X963.hex == "3a17fe5fa33c4f2c7e61799a65061214913f39bfcbee178ab351493d5ee17b2f")
	}

	/// Assert we do not introduce any regression bugs for the custom ECDh `ecdhPoint`
	@Test
	func ecdhCustom() throws {
		...
		let ba = try bob.ecdhPoint(with: alice.publicKey)
		#expect(ab == ba, "Alice and Bob should be able to agree on the same secret")
		#expect(ab.hex == "041d3e7279da3f845c4246087cdd3dd42bea3dea7245ceaf75609d8eb0a4e89c4e8e7a7c012045a2eae87463012468d7aae911b8a1140e240c828c96d9b19bd8e7")
	}
}
