...
final class ECDHTests: XCTestCase {
	...
	func testECDHX963_safe() throws {
		let alice = K1.KeyAgreement.SafePrivateKey()
		let bob = K1.KeyAgreement.SafePrivateKey()
		let ab = try alice.sharedSecretFromKeyAgreement(with: bob.publicKey)
		let ba = try bob.sharedSecretFromKeyAgreement(with: alice.publicKey)
		ab.withUnsafeBytes {
			XCTAssertEqual(Data($0).count, 32)
	...
	func test_crypto_stackexchange_vector() throws {
		let privateKey1 = try K1.KeyAgreement.UnsafePrivateKey(hex: "82fc9947e878fc7ed01c6c310688603f0a41c8e8704e5b990e8388343b0fd465")
		let privateKey2 = try K1.KeyAgreement.UnsafePrivateKey(hex: "5f706787ac72c1080275c1f398640fb07e9da0b124ae9734b28b8d0f01eda586")
	...
	func testECDHCustom() throws {
		let alice = try K1.KeyAgreement.UnsafePrivateKey(rawRepresentation: Data(repeating: 0xAA, count: 32))
		let bob = try K1.KeyAgreement.UnsafePrivateKey(rawRepresentation: Data(repeating: 0xBB, count: 32))
		let ab = try alice.ecdhPoint(with: bob.publicKey)
		let ba = try bob.ecdhPoint(with: alice.publicKey)
		XCTAssertEqual(ab, ba, "Alice and Bob should be able to agree on the same secret")
