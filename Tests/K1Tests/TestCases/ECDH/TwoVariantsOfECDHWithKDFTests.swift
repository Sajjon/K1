...
@Suite("TwoVariantsOfECDHWithKDFTests")
struct TwoVariantsOfECDHWithKDFTests {
	@Test
	func twoVariantsOfECDHWithKDF_vectors() throws {
...
extension TwoVariantsOfECDHWithKDFTests {
	private func doTest(_ vector: Vector) throws {
		...
		#expect(alice.publicKey.x963Representation.hex == vector.alicePublicKeyUncompressed)
		#expect(bob.publicKey.x963Representation.hex == vector.bobPublicKeyUncompressed)
		...
		for outcome in vector.outcomes {
			...
			case .asn1X963:
				...
				sharedSecretAB.withUnsafeBytes {
					#expect(Data($0).hex == outcome.ecdhSharedKey, "Wrong ECDH secret, mismatched expected from vector.")
				}

				for derivedKeys in outcome.derivedKeys {
					let info = try #require(derivedKeys.info.data(using: .utf8))
					...
					x963.withUnsafeBytes {
						#expect(Data($0).hex == derivedKeys.x963, "Wrong X963 KDF result, mismatched expected from vector.")
					}
					...
					hkdf.withUnsafeBytes {
						#expect(Data($0).hex == derivedKeys.hkdf, "Wrong HKDF result, mismatched expected from vector.")
					}
				}
