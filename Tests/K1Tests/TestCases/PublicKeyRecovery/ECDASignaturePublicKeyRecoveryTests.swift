...
final class ECDASignaturePublicKeyRecoveryTests: XCTestCase {
	...
	func test_conversionRoundtrips() throws {
		...
		try XCTAssertEqual(K1.ECDSAWithKeyRecovery.Signature(compact: compactRecoverableSigRS, recoveryID: recid), K1.ECDSAWithKeyRecovery.Signature(compact: compactRecoverableSig))
...
struct RecoveryTestVector: Decodable, Equatable {
	let recoveryID: K1.ECDSAWithKeyRecovery.Signature.RecoveryID
	let message: String
	let hashMessage: String
	private let signature: String

	func recoverableSignature() throws -> K1.ECDSAWithKeyRecovery.Signature {
