...
All private key types in K1 are divided into `PrivateKey` and `SafePrivateKey` where the former are standard copyable types and the latter are Noncopyable types ("move only").
